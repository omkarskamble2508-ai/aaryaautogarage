...
function AddProduct() {
  ...
  const FIELDS=[
    ...
    {label:"Brand",       name:"brand",     type:"text",   ph:"Enter brand"},
    ...
    {label:"Categories",  name:"categories",type:"text",   ph:"Comma-separated categories"},
    {label:"Price (₹)",   name:"price",     type:"number", ph:"Enter price"},
    {label:"Stock Qty",   name:"stock_quantity",type:"number",ph:"Enter stock quantity"},
  ];
      ...
      <div style={{maxWidth:"560px",margin:"0 auto"}}>
        <div style={{background:A.surface,border:`1.5px solid ${A.border}`,borderRadius:"20px",overflow:"hidden",boxShadow:"0 4px 24px rgba(0,0,0,0.06)"}}>
          <div style={{background:A.bg,padding:"1.5rem 2rem",borderBottom:`1px solid ${A.border}`}}>
            <h2 style={{color:A.text,fontSize:"1.15rem",fontWeight:800,margin:0}}>Add Product</h2>
            <p style={{color:A.muted,fontSize:"0.85rem",margin:"0.3rem 0 0"}}>Add a new spare part to the inventory</p>
          </div>
          <div style={{padding:"2rem"}}>
            <form onSubmit={submit}>
              ...
              <div style={{marginBottom:"1.25rem"}}>
                ...
                {product.image&&<p style={{color:A.success,fontSize:"0.76rem",marginTop:"0.4rem"}}>✓ Uploaded: {product.image}</p>}
              </div>
