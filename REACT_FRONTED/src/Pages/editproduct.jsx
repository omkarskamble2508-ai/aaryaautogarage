...
function Editproduct() {
  ...
  useEffect(()=>{
    axios.get(`http://localhost:3000/Product/${pid}`).then(r=>{
      const d=r.data;
  ...
  const submit=async(e)=>{ e.preventDefault(); if(!validate())return; setLoading(true); try{ await axios.put(`http://localhost:3000/Product/${pid}`,product); navigate("/productlist"); }catch{toast.error("Failed to update product");} finally{setLoading(false);} };
